app.controller(
  ...
  function (
    ...
    $scope.submitCustomerModal = () => {
      let index = $scope.customers.findIndex(
        x => x.customer_ID == $scope.selectedCustomerID
      );
        ...
        sellFactory.submitBarcode($scope.barcodeInput).then(data => {
          if (data) {
            ...
            for (let i = 0; i < $scope.returnInvoice.length; i++) {
              if (
                ($scope.returnInvoice[i].item_ID ||
                  $scope.returnInvoice[i].item_ID_FK) == itemToAdd.item_ID
              ) {
    ...
    $scope.submitNewValue = () => {
      $scope.returnInvoice[selectedInvoiceIndex]["discounted_price"] =
