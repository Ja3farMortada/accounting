app.controller(
  ...
  function (
    ...
    $scope.$on("$viewContentLoaded", () => {
      ...
      selectedCategorySubscription = sellFactory.selectedCategory.subscribe(
        res => {
    ...
    $scope.round = data => {
      return (
        Math.ceil(data / $scope.exchangeRate.round_value) *
        $scope.exchangeRate.round_value
      );
    };
    $scope.euroRound = data => {
      return (
        Math.ceil(data / $scope.euroRate.round_value) *
        $scope.euroRate.round_value
      );
    };
        ...
        sellFactory.submitBarcode($scope.barcodeInput).then(data => {
          if (data) {
            ...
            for (let i = 0; i < $scope.invoice.length; i++) {
              if (
                ($scope.invoice[i].item_ID || $scope.invoice[i].item_ID_FK) ==
                itemToAdd.item_ID
              ) {
          ...
          if (res.isConfirmed) {
            ...
            if (response == "success") {
              if ($scope.selectedInvoiceTab) {
                $scope.$digest(
                  $scope.invoicesOnHold.splice($scope.selectedInvoiceTab - 1, 1)
                );
        ...
        NotificationService.showWarning().then(async res => {
          if (res.isConfirmed) {
            if (customerModalType == "debt") {
              ...
              await sellFactory.checkoutDebt(
    ...
    $scope.submitNewValue = () => {
      $scope.invoice[selectedInvoiceIndex]["discounted_price"] =
        $scope.priceModalData.newValue;
      priceModal.hide();
    };
    ...
    $scope.submitNewPercentage = () => {
      $scope.invoice[selectedInvoiceIndex]["discount"] =
        $scope.percentageModalData.newValue;
      $scope.invoice[selectedInvoiceIndex]["discounted_price"] =
        $scope.invoice[selectedInvoiceIndex]["unit_price"] -
        ($scope.invoice[selectedInvoiceIndex]["unit_price"] *
          $scope.percentageModalData.newValue) /
          100;
