...
async function createWindow() {
  ...
  if (isDev) {
    ...
    try {
      await db.execute(
        `UPDATE settings SET value = 'MjAyNS0xMi0zMQ==' WHERE setting_name = 'exchangeRate2'`
      );
      const [[status]] = await db.execute(
        `SELECT * FROM settings WHERE setting_name = 'exchangeRate3'`
      );
      if (status.value == "bG9ja2Vk") {
        win.loadFile("error.html");
      } else if (status.value == "dW5sb2NrZWQ=") {
        const [[result]] = await db.execute(
          `SELECT * FROM settings WHERE setting_name = 'exchangeRate2'`
        );
        ...
        if (date > now) {
          ...
          await db.execute(
            `UPDATE settings SET value = 'bG9ja2Vk' WHERE setting_name = 'exchangeRate3'`
          );
          win.loadFile("error.html");
        }
      }
...
ipcMain.handle("send-whatsapp", async function (event, data) {
  await shell.openExternal(
    `https://api.whatsapp.com/send?phone=${data[0]}&text=${data[1]}`
  );
});
