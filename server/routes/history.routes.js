module.exports = (app, db) => {
    ...
    app.get('/getSalesInvoices/:date', async (req, res) => {
        ...
        let query = `SELECT I.invoice_ID, I.invoice_type, TIME(I.edited_datetime) AS invoice_time, I.total_cost, I.total_price, C.customer_name AS customer_name, U.owner AS user, 
        JSON_ARRAYAGG(JSON_OBJECT('record_ID', M.record_ID, 'item_ID_FK', M.item_ID_FK, 'item_description', S.item_description, 'qty', M.qty, 'currency', M.currency, 'exchange_rate', M.exchange_rate, 'euro_rate', M.euro_rate, 'discount', M.discount, 'discounted_price', M.discounted_price, 'unit_cost', M.unit_cost, 'original_price', M.original_price, 'unit_price', M.unit_price)) invoice_map FROM invoice AS I
