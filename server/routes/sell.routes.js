module.exports = (app, db) => {
  ...
  app.post("/getBarcode", async (req, res) => {
    ...
    try {
      let [[result]] = await db.query(
        `SELECT S.*, C.category_name FROM stock S INNER JOIN stock_categories C ON S.category_ID_FK = C.category_ID WHERE barcode = ? AND item_status = 1`,
        barcode
      );
      res.send(result);
    } catch (error) {
  ...
  app.post("/checkout", async (req, res) => {
    ...
    try {
      ...
      const [result] = await connection.query(`INSERT INTO invoice SET ?`, [
        invoice,
      ]);
      ...
      await connection.query(
        `INSERT INTO invoice_map (invoice_ID_FK, item_ID_FK, record_type, qty, currency, discount, discounted_price, exchange_rate, euro_rate, unit_cost, original_cost, original_price, unit_price) VALUES ?`,
        [invoice_map]
      );
  ...
  app.post("/confirmEditInvoice", async (req, res) => {
    ...
    try {
      ...
      await connection.query(
        `DELETE FROM invoice_map WHERE invoice_ID_FK = ?`,
        invoice_ID
      );
      ...
      let [[response]] = await db.query(
        `SELECT I.invoice_ID, I.customer_ID_FK AS customer_ID, I.invoice_type, DATE(I.invoice_datetime) AS invoice_date, TIME(I.invoice_datetime) AS invoice_time, I.edited_datetime AS edited_datetime, I.total_cost, I.total_price, C.customer_name AS customer_name, U.owner AS user, 
